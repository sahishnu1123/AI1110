...
//Defining function for generating V random variables
...
//Defining function for calculating varience of random numbers
double varience(char *str)
{
int i=0,c;
FILE *fp;
double x, mean_of_squares=0.0;
double u = mean(str);
fp = fopen(str,"r");
while(fscanf(fp,"%lf",&x)!=EOF)
{
i=i+1;
mean_of_squares = mean_of_squares + x*x;
}
fclose(fp);
mean_of_squares = mean_of_squares/(i-1);
double temp = mean_of_squares - u*u;

return temp;
}
//End function for generating varience of random variables

//Defining function for generating T random variables
void rand_T(char *str, int len)
{
int i;
FILE *fp;
...
for (i = 0; i < len; i++)
{
double temp = (double)rand()/RAND_MAX + (double)rand()/RAND_MAX;
fprintf(fp,"%lf\n",temp);
}
fclose(fp);

}
//End function for generating T random numbers
